#include "StdAfx.h"
#include "TrayLoadPPCmd_VatBacklash.h"
...
//
//==============================================================================
CTrayLoadPPCmd_VatBacklash::CTrayLoadPPCmd_VatBacklash(CDlgPcdBacklash *pParent)
{
...
// 	m_dTrayPP_Z_Safety = m_Teaching_Data.GetTeachPos(40, 9, 4);
	...
	m_nRetryCount = 0;
	sprintf_s(m_szCmdName, sizeof(m_szCmdName), "CTrayLoadPPCmd_VatBacklash");
...
//
//==============================================================================
CTrayLoadPPCmd_VatBacklash::~CTrayLoadPPCmd_VatBacklash(void){}

//==============================================================================
//
//==============================================================================
int CTrayLoadPPCmd_VatBacklash::CheckParam()
{
	...
	//OneAxis* pMotorY = pTaskTrayLoadPP->GetAxis(CTaskTrayLoadPP::eAxisY);
...
//
//==============================================================================
int  CTrayLoadPPCmd_VatBacklash::OnStop()
{
...
//
//==============================================================================
int  CTrayLoadPPCmd_VatBacklash::OnRetry()
{
...
//
//==============================================================================
int  CTrayLoadPPCmd_VatBacklash::OnSkip()
{
...
//
//==============================================================================
int  CTrayLoadPPCmd_VatBacklash::OnPause()
{
...
//
//==============================================================================
int  CTrayLoadPPCmd_VatBacklash::OnResume()
{
	SetCmdStatus(CBaseSeqCmd::eSeqCmdStatus_Recovery);
	return ERR_NO_ERROR;
}
...
//
//==============================================================================
BOOL CTrayLoadPPCmd_VatBacklash::State_Setup()
{
...
//
//==============================================================================
BOOL CTrayLoadPPCmd_VatBacklash::State_Executing()
{
	int nErr = 0;
...
// 		pTaskTrayLoadPP->MakeLog("[CMD: %s] [EXECUTE] [Step=%8d]", m_szCmdName, m_nStep);
// 	}
	m_nStep_Pre = m_nStep;
	...
	case exe_backlash_loop_entry:
		{
			...
			pTaskTrayLoadPP->MakeLog("[%s] [EXEC=%8d][Backlash Start Count : %d, Speed : %d ]",m_szCmdName ,m_nStep,m_nChkLoopCnt, m_nSpeed);
	...
	case exe_hand_z_safety_move:
		{
			...
			NextStepTimer( exe_hand_z_safety_move_check, TIME_OUT_SHORT);
		}break;
	case exe_hand_z_safety_move_check:
		{
			m_vErrPicker.clear();
			int nAxisDone = pMotorZ->AxisDone(m_vSafetyAllPicker, m_vErrPicker);
			int nErrSen = pTaskTrayLoadPP->Chk_Z_Safety_Sen_Check_Type(m_vSafetyAllPicker, ePickerSafetySensorCheckType::eSafetySensorCheck);
			if (nAxisDone == ERR_NO_ERROR && nErrSen == ERR_NO_ERROR) 
			{
				NextStep(exe_hand_xy_pitch_safety_move);
				break;
			}
			else {
	...
	case exe_hand_xy_pitch_safety_move_check:
	{
		...
		if (nChkComp == _countof(nAxisNo)) {
			...
			m_tcTimeoutPicker.SetCheckTime(TIME_OUT_MIDDLE);
			m_tcTimeoutPicker.StartTimer();
			m_nStep = exe_hand_xy_move_target_position;
			break;
		}
	...
	case exe_hand_xy_move_target_position:
		{
			...
			double dTPosX[1] = {0.0,};
			dTPosX[0] = g_VatBacklash.trayloadpp.m_BacklashTargetPos_XYZ.x;

			pTaskTrayLoadPP->MakeLog("[%s] [EXEC=%8d] [x Start Position move] [SRT]", m_szCmdName, m_nStep);

			//Tray Load Hand X
			for(int i=0; i<_countof(nAxisX); i++){
				int nErr = pTaskTrayLoadPP->do_MovePos_Pcd_backlash(vPicker, dTPosX[i], (CTaskTrayBasePP::eMotor)nAxisX[i], m_nSpeed, 0, m_bBacklashTest);
				if( nErr != ERR_NO_ERROR ){
					CParamList AlarmData;
			...
			int nAxisY[1] = { CTaskTrayFeeder::eMotor::eAxisY };

			double dTPosY[1] = { 0.0, };
			dTPosY[0] = g_VatBacklash.trayloadpp.m_BacklashTargetPos_XYZ.y;

			pTaskTrayLoadPP->MakeLog("[%s] [EXEC=%8d] [feeder y Start Position move] [SRT]", m_szCmdName, m_nStep);

			//Feeder Y
			for (int i = 0; i < _countof(nAxisY); i++) {
				int nErr = g_TaskTrayFeeder.do_MovePos_Pcd_backlash(dTPosY[i], (CTaskTrayFeeder::eMotor)nAxisY[i], m_nSpeed, 0, m_bBacklashTest);
				if (nErr != ERR_NO_ERROR) {
					CParamList AlarmData;
	...
	case exe_hand_xy_move_target_position_check:
		{
			...
			if (nChkComp == _countof(pMotor)) {
				pTaskTrayLoadPP->MakeLog("[%s] [EXEC=%8d] [x,y start position] [END]", m_szCmdName, m_nStep);
	...
	case exe_hand_z_move_target_safety_position:
		{
			...
			NextStepTimer( exe_hand_z_move_target_safety_position_check, TIME_OUT_SHORT);
		}break;
	...
	case exe_hand_xy_move_target_to_start_position:
		{
			...
			double dTPosX[1] = {0.0,};
			dTPosX[0] = g_VatBacklash.trayloadpp.m_BacklashStartPos_XY.x;
			...
			double dTPosY[1] = { 0.0, };
			dTPosY[0] = g_VatBacklash.trayloadpp.m_BacklashStartPos_XY.y;
	...
	case exe_hand_xy_move_start_to_target_position:
		{
			...
			pTaskTrayLoadPP->MakeLog("[%s] [EXEC=%8d] [x Target Position move] [SRT]", m_szCmdName, m_nStep);
			for(int i=0; i<_countof(nAxisX); i++){
				int nErr = pTaskTrayLoadPP->do_MovePos_Pcd_backlash(vPicker, dTPosX[i], (CTaskTrayBasePP::eMotor)nAxisX[i], m_nSpeed, 0, m_bBacklashTest);
				if( nErr != ERR_NO_ERROR ){
					CParamList AlarmData;
	...
	case exe_hand_xy_move_start_to_target_position_check:
		{
			...
			if (nChkComp == _countof(pMotor)) {
				pTaskTrayLoadPP->MakeLog("[%s] [EXEC=%8d] [x,y Target position] [END]", m_szCmdName, m_nStep);
	...
	case exe_hand_z_move_start_to_target_position:
		{
			double dTPos = g_VatBacklash.trayloadpp.m_BacklashTargetPos_XYZ.z;
	...
	case exe_visn_recon_start_to_target_pos:
		{
			pTaskTrayLoadPP->MakeLog("[%s] [EXEC=%8d] [Backlash Start to Target measurement] [SRT]", m_szCmdName, m_nStep);
			if( is_simulation() ){
	...
	case exe_visn_recon_start_to_target_pos_check:
		{
			...
			if( CheckMeasureResult(vRcvData) == TRUE ){
				pTaskTrayLoadPP->MakeLog("[%s] [EXEC=%8d] [Backlash Speed:%d Start To Target measurement dx=%.3f um, dy=%.3f um] [END]", m_szCmdName, m_nStep,m_nSpeed,vRcvData.dX,vRcvData.dY);
	...
	case exe_hand_z_start_to_target_safety_move:
		{
			pTaskTrayLoadPP->MakeLog("[%s] [EXEC=%8d] [z move to safety position] [SRT]", m_szCmdName, m_nStep);
			...
			NextStepTimer( exe_hand_z_start_to_target_safety_move_check, TIME_OUT_SHORT );
		}break;
	...
	case exe_hand_xy_move_target_to_end_position:
		{
			...
			double dTPosX[1] = {0.0,};
			dTPosX[0] = g_VatBacklash.trayloadpp.m_BacklashEndPos_XY.x;

			pTaskTrayLoadPP->MakeLog("[%s] [EXEC=%8d] [x End Position move] [SRT]", m_szCmdName, m_nStep);
			//Tray Load Hand X
			for(int i=0; i<_countof(nAxisX); i++){
				int nErr = pTaskTrayLoadPP->do_MovePos_Pcd_backlash(vPicker, dTPosX[i], (CTaskTrayBasePP::eMotor)nAxisX[i], m_nSpeed, 0, m_bBacklashTest);
				if( nErr != ERR_NO_ERROR ){
			...
			double dTPosY[1] = { 0.0, };
			dTPosY[0] = g_VatBacklash.trayloadpp.m_BacklashEndPos_XY.y;

			pTaskTrayLoadPP->MakeLog("[%s] [EXEC=%8d] [feeder y End Position move] [SRT]", m_szCmdName, m_nStep);
			//Feeder Y
			for (int i = 0; i < _countof(nAxisY); i++) {
				int nErr = g_TaskTrayFeeder.do_MovePos_Pcd_backlash(dTPosY[i], (CTaskTrayFeeder::eMotor)nAxisY[i], m_nSpeed, 0, m_bBacklashTest);
				if (nErr != ERR_NO_ERROR) {
	...
	case exe_hand_xy_move_target_to_end_position_check:
		{
			...
			if (nChkComp == _countof(pMotor)) {
				pTaskTrayLoadPP->MakeLog("[%s] [EXEC=%8d] [x,y End position] [END]", m_szCmdName, m_nStep);
	...
	case exe_visn_recon_end_to_target_pos:
		{
			pTaskTrayLoadPP->MakeLog("[%s] [EXEC=%8d] [Backlash End to Target measurement] [SRT]", m_szCmdName, m_nStep);
			if( is_simulation() ){
	...
	case exe_hand_z_end_to_target_safety_move:
		{
			...
			NextStepTimer( exe_hand_z_end_to_target_safety_move_check, TIME_OUT_SHORT );
		}break;
	...
	case exe_complete:
		{
			...
			CParamList AlarmData;
			AlarmData.m_fnAddItemFormat("Tray Load PP Backlash complete");
			...
			if(m_bBacklashTest)
			{
				...
				for(int i=0; i<nLength; i++)
				{
					...
					strBacklashOfs = GetBacklashAxisInfo(m_nAxisNum, i, m_vBacklashAdjust[i].x);
...
//
//==============================================================================
BOOL CTrayLoadPPCmd_VatBacklash::State_Pause()
{
	CTaskTrayLoadPP* pTaskTrayLoadPP = (CTaskTrayLoadPP*)m_pReceive;
	if( m_nStep_New != -1 ){
		m_nStep = m_nStep_New;
...
//
//==============================================================================
BOOL CTrayLoadPPCmd_VatBacklash::State_Recovery()
{
	CTaskTrayLoadPP* pTaskTrayLoadPP = (CTaskTrayLoadPP*)m_pReceive;
	if( m_nStep_New != -1 ){
		m_nStep = m_nStep_New;
...
//
//==============================================================================
BOOL CTrayLoadPPCmd_VatBacklash::State_NormalComplete()
{
...
//
//==============================================================================
BOOL CTrayLoadPPCmd_VatBacklash::State_Alarm()
{
	CTaskTrayLoadPP* pTaskTrayLoadPP = (CTaskTrayLoadPP*)m_pReceive;
	if( m_nStep_New != -1 ){
		m_nStep = m_nStep_New;
...
//
//==============================================================================
BOOL CTrayLoadPPCmd_VatBacklash::State_AlarmComplete()
{
...
//
//==============================================================================
BOOL CTrayLoadPPCmd_VatBacklash::State_Aborting()
{
	CTaskTrayLoadPP* pTaskTrayLoadPP = (CTaskTrayLoadPP*)m_pReceive;
...
//
//==============================================================================
BOOL CTrayLoadPPCmd_VatBacklash::State_AbortComplete()
{
...
//==============================================================================
void CTrayLoadPPCmd_VatBacklash::NextStep( execute_step_define next )
{
	m_nStep = next;
}

void CTrayLoadPPCmd_VatBacklash::NextStepTimer( execute_step_define next , double tm )
{
...
void CTrayLoadPPCmd_VatBacklash::SimpleAlarm( int alarm_id )
{
	CParamList AlarmData;
	ReportAlarm( alarm_id, AlarmData );
}

BOOL CTrayLoadPPCmd_VatBacklash::CheckMeasureResult( ST_PCD_BACKLASH_RCV_DATA& result )
{
	if( m_tcTimeout.CheckOverTime() == TRUE ){
		CParamList AlarmData;
...
void CTrayLoadPPCmd_VatBacklash::CalcBacklashOffset(int nSpeed, BOOL bIsBacklashTest)
{
	CPointF Temp;
	...
	if(bIsBacklashTest == false)
	{
		...
		if(nIdx < eSpeed_MAX)
		{
			...
			if (m_nAxisNum == eAxis_X) {
				g_VatBacklash.trayloadpp.m_BacklashOffset_XY[nIdx].x = Temp.x /** -1*/;
				m_BacklashXTestLog.LOG(" SPD -> %3d / OFFSET -> %.5f", nSpeed, g_VatBacklash.trayloadpp.m_BacklashOffset_XY[nIdx].x);
			}
			else if(m_nAxisNum == eAxis_Y){
				g_VatBacklash.trayloadpp.m_BacklashOffset_XY[nIdx].y = Temp.y /** -1*/;
				m_BacklashYTestLog.LOG(" SPD -> %3d / OFFSET -> %.5f", nSpeed, g_VatBacklash.trayloadpp.m_BacklashOffset_XY[nIdx].y);
			}
...
CString CTrayLoadPPCmd_VatBacklash::GetBacklashAxisInfo(int nAxis, int nSpdIndex, double dOffset)
{
	CString str = _T("");
	int nSpd = 0;
	if(nSpdIndex == 0)
...
void CTrayLoadPPCmd_VatBacklash::InitLoopStatus()
{
...
void CTrayLoadPPCmd_VatBacklash::WriteRepeatOffsetXY(std::vector<CPointF> End2TargetOfs, std::vector<CPointF> Start2TargetOfs, int spd)
{
	if(m_bRepeat == FALSE)
	{
		return;
...
void CTrayLoadPPCmd_VatBacklash::WriteMotorEncoder(int speed)
{
	OneAxis* pMotorX  = g_TaskTrayLoadPP.GetAxis(CTaskTrayLoadPP::eAxisX);
	//OneAxis* pMotorY  = g_TaskTrayLoadPP.GetAxis(CTaskTrayLoadPP::eAxisY);
	...
	sprintf_s(szData2, sizeof(szData2), "%.5f\r\n", dCurPosX);
	MotorEnc.SeekToEnd();
