...
class CDlgAlarmMsgBoxDoor : public CDialogEx
{
	DECLARE_DYNAMIC(CDlgAlarmMsgBoxDoor)

public:
	CDlgAlarmMsgBoxDoor(ST_ALARM_RPT_OBJ* pstAlarmObj, CParamList* pAlarmData = NULL, CWnd* pParent = NULL);   // 표준 생성자입니다.
	virtual ~CDlgAlarmMsgBoxDoor();

	// 대화 상자 데이터입니다.
	enum {IDD = IDD_ALARM_MSG_DOOR};
	enum {
...
private:
	...
	std::vector<int> m_vErrorPos;
	CSpreadSheet m_SprdPicker;
	CStatic m_bitPic1;
	CEditTrans m_editPosition[eMaxDoorSensor];
	BOOL bIsRed[eMaxDoorSensor];
