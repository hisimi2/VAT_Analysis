...
static int m_nPosicionControl[ePos_MaxPosition - 1] = { IDC_EDIT_0 ,IDC_EDIT_1,IDC_EDIT_2,IDC_EDIT_3,IDC_EDIT_4,IDC_EDIT_5,IDC_EDIT_6,IDC_EDIT_7,IDC_EDIT_8,IDC_EDIT_9,IDC_EDIT_10
											, IDC_EDIT_11, IDC_EDIT_12, IDC_EDIT_13,IDC_EDIT_14, IDC_EDIT_15, IDC_EDIT_16, IDC_EDIT_17, IDC_EDIT_18, IDC_EDIT_19, IDC_EDIT_20
											, IDC_EDIT_21, IDC_EDIT_22, IDC_EDIT_23, IDC_EDIT_24, IDC_EDIT_25,IDC_EDIT_26 };

// CDlgAlarmMsgBox 대화 상자입니다.

IMPLEMENT_DYNAMIC(CDlgAlarmMsgBox, CDialogEx)

CDlgAlarmMsgBox::CDlgAlarmMsgBox(ST_ALARM_RPT_OBJ* pstAlarmObj,CParamList* pAlarmData, CWnd* pParent /*=NULL*/)
	: CDialogEx(CDlgAlarmMsgBox::IDD, pParent)
{
...
CDlgAlarmMsgBox::~CDlgAlarmMsgBox()
{

}

void CDlgAlarmMsgBox::DoDataExchange(CDataExchange* pDX)
{
	CDialogEx::DoDataExchange(pDX);
	DDX_Control(pDX, IDC_SPRD_PICKER, m_SprdPicker);
	DDX_Control(pDX, IDC_STATIC_PICTURE, m_bitPic1);
...
BEGIN_MESSAGE_MAP(CDlgAlarmMsgBox, CDialogEx)
	ON_WM_DESTROY()
	ON_WM_SHOWWINDOW()
	ON_WM_TIMER()
	ON_BN_CLICKED(IDOK, &CDlgAlarmMsgBox::OnBnClickedOk)
	ON_BN_CLICKED(ID_BUTTON_USER_SEL_1, &CDlgAlarmMsgBox::OnBnClickedButtonUserSel1)
	ON_BN_CLICKED(ID_BUTTON_USER_SEL_2, &CDlgAlarmMsgBox::OnBnClickedButtonUserSel2)
	ON_BN_CLICKED(ID_BUTTON_USER_SEL_3, &CDlgAlarmMsgBox::OnBnClickedButtonUserSel3)
	ON_BN_CLICKED(ID_BUTTON_USER_SEL_4, &CDlgAlarmMsgBox::OnBnClickedButtonUserSel4)
	ON_BN_CLICKED(ID_BUTTON_SET_MBTYPE, &CDlgAlarmMsgBox::OnBnClickedButtonSetMbtype)
	ON_WM_ACTIVATE()
END_MESSAGE_MAP()


// CDlgAlarmMsgBox 메시지 처리기입니다.


BOOL CDlgAlarmMsgBox::PreTranslateMessage(MSG* pMsg)
{
...
void CDlgAlarmMsgBox::DisplayErrMsg()
{
	CString strAlid;
...
void CDlgAlarmMsgBox::InitControls()
{
	...
	m_nPosition = m_tErrorMsg.nPosition;
}


BOOL CDlgAlarmMsgBox::OnInitDialog()
{
...
// 		AllocConsole();  // 콘솔 할당
// 
// 						 // C 표준 입출력 스트림을 콘솔에 연결
// 		FILE* pCout;
// 		freopen_s(&pCout, "CONOUT$", "w", stdout);  // printf 출력 연결
// 		freopen_s(&pCout, "CONOUT$", "w", stderr);  // fprintf(stderr, ...) 연결
// 		freopen_s(&pCout, "CONIN$", "r", stdin);    // scanf 등 입력 연결
// 
// 		std::ios::sync_with_stdio();  // C++ 입출력 동기
...
void CDlgAlarmMsgBox::OnDestroy()
{
	CDialogEx::OnDestroy();
...
void CDlgAlarmMsgBox::OnShowWindow(BOOL bShow, UINT nStatus)
{
...
void CDlgAlarmMsgBox::OnTimer(UINT_PTR nIDEvent)
{
	switch (nIDEvent)
	{
		case 1:
...
void CDlgAlarmMsgBox::InitUserSelectionButton(int nMBTYPE,int nAlid)
{
...
void CDlgAlarmMsgBox::UserSelection(int nSelection)
{
	// Recovery 설정
	CString strRecovery;
	int nCmd;
...
void CDlgAlarmMsgBox::OnBnClickedOk()
{
	UserSelection( eUserSelect_OK );
	//CDialogEx::OnOK();
}

void CDlgAlarmMsgBox::PostNcDestroy()
{
...
int CDlgAlarmMsgBox::ConvertUserSelection(CString strUserSelection)
{
...
void CDlgAlarmMsgBox::OnBnClickedButtonUserSel(int nButtonIdx)
{
...
void CDlgAlarmMsgBox::OnBnClickedButtonUserSel1()
{
	OnBnClickedButtonUserSel(0);
}


void CDlgAlarmMsgBox::OnBnClickedButtonUserSel2()
{
	OnBnClickedButtonUserSel(1);
}


void CDlgAlarmMsgBox::OnBnClickedButtonUserSel3()
{
	OnBnClickedButtonUserSel(2);
}


void CDlgAlarmMsgBox::OnBnClickedButtonUserSel4()
{
	OnBnClickedButtonUserSel(3);
}


void CDlgAlarmMsgBox::OnBnClickedButtonSetMbtype()
{
...
void CDlgAlarmMsgBox::InitSpread()
{
	char szCompany[16] = { 0, };
	g_DMCont.m_dmHandlerOpt.GS(SDM11_Company, szCompany, sizeof(szCompany));
...
// 			m_SprdPicker.SetMaxCols(nPocketCount_X);
// 		}
		else if (m_nAlid >= ERR_LOAD_TBL_1_VISION_BARCODE_IS_SHORT && m_nAlid <= ERR_LOAD_TBL_4_VISION_BARCODE_MARK_INVALIDATE) {
			...
			for (int i = 0; i <= nPocketCntY; i++) {
				pSprd->SetRowHeight(i, 40);
			}

			for (int i = 0; i <= nPocketCntX; i++) {
				pSprd->SetColWidth(i, 3);
			}

			// 고객만족팀 요청 사항 Table Error Pocket 가로 -> 세로 방향으로 표시 하도록 변경
			if (m_nPosition == ePos_LoadTbl_2)
			{
				LoadTableErrorPocketNumberSet(nBoardType == eBoardDir_Forward ? _eCCW : _eCW, &m_SprdPicker);
				nLoadTableIdx = eLoadTbl_2;
			}
			else if (m_nPosition == ePos_LoadTbl_3)
			{
				LoadTableErrorPocketNumberSet(nBoardType == eBoardDir_Forward ? _eCW : _eCCW, &m_SprdPicker);
				nLoadTableIdx = eLoadTbl_3;
			}

			if (m_nPosition == ePos_LoadTbl_2 || m_nPosition == ePos_LoadTbl_3)
			{
				if (m_vFailPicker.size() > 0)
				{
					std::vector<std::pair<CPoint, CPoint>> vPocketLocChange = g_TaskLoadTbl[nLoadTableIdx].GetPocketLocNum();
					std::vector<CPoint> vPicker; vPicker.clear();
					for (const auto& p : vPocketLocChange) {
						for (const auto& i : m_vFailPicker)
						{
							if (p.first.x == i.x && p.first.y == i.y)
							{
								vPicker.push_back(CPoint(p.second.x, p.second.y));
								break;
							}
						}
					}
					m_vFailPicker.clear();
					m_vFailPicker = vPicker;
				}
			}		
		}
			...
			for (int i = 0; i <= nPocketCntY; i++) {
				pSprd->SetRowHeight(i, 20);
			}
				
			for (int i = 0; i <= nPocketCntX; i++) {
				pSprd->SetColWidth(i, 10);
			}
...
void CDlgAlarmMsgBox::InitReSize()
{
...
void CDlgAlarmMsgBox::InitPosition()
{
...
BOOL CDlgAlarmMsgBox::IsRightCompany(char * pCompany) 
{
	char szCompany[16] = { 0, };
	g_DMCont.m_dmHandlerOpt.GS(SDM11_Company, szCompany, sizeof(szCompany));
...
BOOL CDlgAlarmMsgBox::UserSelect(int nAlid,int nSelection)
{
	// [11/17/2020 jaehoon.jung]
...
void CDlgAlarmMsgBox::LoadTableErrorPocketNumberSet(_eCw_CCw bCW, CSpreadSheet* SprdPocket)
{
	int nPocketCntX = g_DMCont.m_dmShuttle.GN(NDM3_LdTbl_Div_X);
	int nPocketCntY = g_DMCont.m_dmShuttle.GN(NDM3_LdTbl_Div_Y);
	CString strNum;
	if (bCW == _eCw_CCw::_eCW)
	{
		//가로 	
		for (int i = 0; i < nPocketCntX; i++) {
			strNum.Format("%d", nPocketCntX - i);
			SprdPocket->SetTextMatrix(0, i + 1, strNum);
		}
	}
	else {
		//세로
		for (int i = 0; i < nPocketCntY; i++) {
			strNum.Format("%d", nPocketCntY - i);
			SprdPocket->SetTextMatrix(i + 1, 0, strNum);
		}
	}
}
