#include "StdAfx.h"
#include "VisionComm_TestHand_2.h"
...
CVisionComm_TestHand_2::CVisionComm_TestHand_2(int nVisionPcIdx, CString strVisionName)
...
CVisionComm_TestHand_2::CVisionComm_TestHand_2(void)
{
	memset(m_nIsReceived, 0, sizeof(m_nIsReceived));
	m_bVisionComm = TRUE;
...
ST_MESSAGE_MAP CVisionComm_TestHand_2::MessageMap[] ={
	{2,3,   CVisionComm_TestHand_2::OnReceiveS2F3 },
	{2,4,   CVisionComm_TestHand_2::OnReceiveS2F4 },
	{2,41,  CVisionComm_TestHand_2::OnReceiveS2F41 },
	{2,42,  CVisionComm_TestHand_2::OnReceiveS2F42 },
	{5, 1,  CVisionComm_TestHand_2::OnReceiveS5F1  },
	{5, 2,  CVisionComm_TestHand_2::OnReceiveS5F2  },
	{6,11,  CVisionComm_TestHand_2::OnReceiveS6F11 },
	{6,12,  CVisionComm_TestHand_2::OnReceiveS6F12 },
	{107,1, CVisionComm_TestHand_2::OnReceiveS107F1 },
	{107,2, CVisionComm_TestHand_2::OnReceiveS107F2 },
	{107,3, CVisionComm_TestHand_2::OnReceiveS107F3 },
	{107,4, CVisionComm_TestHand_2::OnReceiveS107F4 },
	{107,5, CVisionComm_TestHand_2::OnReceiveS107F5 },
	{107,6, CVisionComm_TestHand_2::OnReceiveS107F6 },
	{ 107,9, CVisionComm_TestHand_2::OnReceiveS107F9 },
	{-1,-1, NULL},
};


int CVisionComm_TestHand_2::Initialize(CWnd* pParentWnd, char* szDriverFileName)
{
	int nSocketType = 0;
	char szIp[128] = {0,};
	int nPort = 0;
	...
	sprintf_s(szDefault, sizeof(szDefault), "VisionComm_TestHand2.log");
	ConfigData.GetString( "MC TO VISION COMM CONFIG", "TestHand2 Log File Name", szDefault, szBuff);
	...
	int nRet = CIPCSock::Initialize(pParentWnd, nSocketType, szIp, nPort, CVisionComm_TestHand_2::MessageMap, szLogFileName);
	SendLogMessage("IPC Vision TestHand2 Comm Initialized. [ERR=%d]", nRet);
...
void CVisionComm_TestHand_2::Finalize()
{
	SendLogMessage("IPC Vision Test Hand 2 Comm Finalized.");
	CIPCSock::Finalize();
}


LRESULT CVisionComm_TestHand_2::OnTWIPCEvent(WPARAM wParam, LPARAM lParam)
{
	...
	case IPC_EVT_CONNECTED:
		{
			TRACE("TestHand2 Vision Connected.\n");
			SendLogMessage("TestHand2 Vision Connected");
			KillTimer(0);
			SendLogMessage("TestHand2 Vision KillTimer");
			Sleep(1000); //Vision 통신 연결 후 1초 Sleep 이후에, Set cok 
			Cmd_SetCok();
		}break;
	case IPC_EVT_DISCONNECTED:
		{
			TRACE("TestHand2 Vision Disconnected.\n");
			SendLogMessage("TestHand2 Vision Disconnected");
			m_csCmdDone.Lock();
			...
			//VAT
			for (int nCam = VAT_CAM_LD_TEST_PP_2_UPPER; nCam <= VAT_CAM_TEST_PP_2_LOWER; nCam++)
			{
...
//
//==============================================================================
int CVisionComm_TestHand_2::OnReceiveS2F3(CPacket* pPacket)
{
	CPacketBody_S2F4* pBody = (CPacketBody_S2F4*)pPacket->pBody;
	
	g_pVisionTestHand_2->SendLogMessage("[rcv] S2F4. MSGID : %d, DataCount=%d", pPacket->pHead->stData.nMsgId, pBody->stData.nDataCount);
	...
	for(int i=0; i<pBody->stData.nDataCount; i++){
		g_pVisionTestHand_2->SendLogMessage("      Data %d = %s", i + 1, pBody->stData.szDataValue[i]);
...
//
//========================================================================================
int CVisionComm_TestHand_2::OnReceiveS2F4(CPacket* pPacket)
{
	CPacketBody_S2F4* pBody = (CPacketBody_S2F4*)pPacket->pBody;

	int nReqCount =0;

	g_pVisionTestHand_2->m_csCmdDone.Lock();
	//g_pTestSiteInspUp->m_csCmdDone.Lock();

	int nIndx = 0;
	for (int i = 4; i<eMaxPressUnitCount; i++)
	{
		//if( g_pTestSiteInspUp->m_VisionMsg[i].nMsgId == pPacket->pHead->stData.nMsgId){
		if(g_pVisionTestHand_2->m_VisionMsg[i].nMsgId == pPacket->pHead->stData.nMsgId )
		{
			nIndx = i;
			//nReqCount = g_pTestSiteInspUp->m_VisionMsg[i].nDataCount = pBody->stData.nDataCount;
			nReqCount = g_pVisionTestHand_2->m_VisionMsg[i].nDataCount = pBody->stData.nDataCount;
			g_pVisionTestHand_2->m_VisionMsg[i].nStatus = pBody->stData.nStatus;
			for(int j=0; j<nReqCount; j++)
			{
				//strcpy_s(g_pTestSiteInspUp->m_VisionMsg[i].szData[j], pBody->stData.szDataValue[j]);
				strcpy_s(g_pVisionTestHand_2->m_VisionMsg[i].szData[j], pBody->stData.szDataValue[j]);
			}
			//g_pTestSiteInspUp->m_VisionMsg[i].nReceived = eVisionMsgWaitState_Received;
			g_pVisionTestHand_2->m_VisionMsg[i].nReceived = eVisionMsgWaitState_Received;
...
// 		}
	...
	//g_pTestSiteInspUp->m_csCmdDone.Unlock();
	g_pVisionTestHand_2->m_csCmdDone.Unlock();
	...
	g_pVisionTestHand_2->SendLogMessage("[rcv] S2F4. MSGID : %d, DataCount=%d, vReqDataCount = %d", pPacket->pHead->stData.nMsgId, pBody->stData.nDataCount, nReqCount);
	for(int i=0; i<pBody->stData.nDataCount; i++){
		//g_pTestSiteInspUp->SendLogMessage("      Data %d = %s", i+1, pBody->stData.szDataValue[i]);
		g_pVisionTestHand_2->SendLogMessage("      Data %d = %s", i + 1, pBody->stData.szDataValue[i]);
	}
	for(int i=0; i< nReqCount; i++){
		...
		g_pVisionTestHand_2->SendLogMessage("vReqData Data %d = %s", i + 1, g_pVisionTestHand_2->m_VisionMsg[nIndx].szData[i]);
	}
...
/*=============================================================================
  ...
  COMMENTS : 
==============================================================================*/
int CVisionComm_TestHand_2::OnReceiveS2F41(CPacket* pPacket)
{
	...
	g_pVisionTestHand_2->SendLogMessage("[TestSiteUp rcv] S2F41. MSGID : %d, DataCount=%d", pPacket->pHead->stData.nMsgId, pBody->stData.nCmd);
	for(int i=0; i<pBody->stData.nParamCount; i++){
		//g_pTestSiteInspUp->SendLogMessage("      Data %d = %s", i+1, pBody->stData.szParam[i]);
		g_pVisionTestHand_2->SendLogMessage("      Data %d = %s", i + 1, pBody->stData.szParam[i]);
	}
	...
	g_pVisionTestHand_2->Send(pRePacket, FALSE);
	...
	g_pVisionTestHand_2->SendLogMessage("[snd] TestSiteDown OnReceiveS2F41 Ack S2F42, MsgId : %d, RCMDACK=%d", pRePacket->pHead->stData.nMsgId, pReBody->stData.nRCMDACK);
	return 0;
}
...
/*=============================================================================
  ...
  COMMENTS : 
==============================================================================*/
int CVisionComm_TestHand_2::OnReceiveS2F42(CPacket* pPacket)
{
	...
	if (pPacket->pHead->stData.nMsgId == VISION_RCMD_REQ_BARCORDE_LIGHT_ON_OFF) {
		g_pVisionTestHand_2->m_csCmdDone.Lock();
		g_pVisionTestHand_2->m_BarcodeLightMsg.nRcmdAck = pBody->stData.nRCMDACK;
		g_pVisionTestHand_2->m_BarcodeLightMsg.nReceived = eVisionMsgWaitState_Received;
		g_pVisionTestHand_2->m_csCmdDone.Unlock();
		return 0;
	}
	...
	for(int i=0; i<8; i++){
		...
		if(g_pVisionTestHand_2->m_VisionMsg[i].nMsgId == pPacket->pHead->stData.nMsgId)
		{
			...
			g_pVisionTestHand_2->m_VisionMsg[i].nRcmdAck = pBody->stData.nRCMDACK;
			//g_pTestSiteInspUp->m_VisionMsg[i].nReceived = eVisionMsgWaitState_Received;
			g_pVisionTestHand_2->m_VisionMsg[i].nReceived = eVisionMsgWaitState_Received;
		}
	}
	...
	g_pVisionTestHand_2->SendLogMessage("[rcv] S2F42. MSGID : %d, RCMDACK=%d", pPacket->pHead->stData.nMsgId, pBody->stData.nRCMDACK);
	return 0;
}
...
/*=============================================================================
  ...
  COMMENTS : 
==============================================================================*/
int CVisionComm_TestHand_2::OnReceiveS5F1 (CPacket* pPacket)
{
	return 0;
}
...
/*=============================================================================
  ...
  COMMENTS : 
==============================================================================*/
int CVisionComm_TestHand_2::OnReceiveS5F2 (CPacket* pPacket)
{
	return 0;
}
...
/*=============================================================================
  ...
  COMMENTS : 
==============================================================================*/
int CVisionComm_TestHand_2::OnReceiveS6F11(CPacket* pPacket)
{
	return 0;
}
...
/*=============================================================================
  ...
  COMMENTS : 
==============================================================================*/
int CVisionComm_TestHand_2::OnReceiveS6F12(CPacket* pPacket)
{
	return 0;
}
...
/*=============================================================================
  ...
  COMMENTS : 
==============================================================================*/
int CVisionComm_TestHand_2::OnReceiveS107F1(CPacket* pPacket)
{
	return 0;
}
...
/*=============================================================================
  ...
  COMMENTS : 
==============================================================================*/
int CVisionComm_TestHand_2::OnReceiveS107F2(CPacket* pPacket)
{
	return 0;
}
...
/*=============================================================================
  ...
  COMMENTS : 
==============================================================================*/
int CVisionComm_TestHand_2::OnReceiveS107F3(CPacket* pPacket)
{
	return 0;
}
...
/*=============================================================================
  ...
  COMMENTS : 
==============================================================================*/
int CVisionComm_TestHand_2::OnReceiveS107F4(CPacket* pPacket)
{
	return 0;
}
...
/*=============================================================================
  ...
  COMMENTS : 
==============================================================================*/
int CVisionComm_TestHand_2::OnReceiveS107F5(CPacket* pPacket)
{
	return 0;
}
...
/*=============================================================================
  ...
  COMMENTS : 
==============================================================================*/
int CVisionComm_TestHand_2::OnReceiveS107F6(CPacket* pPacket)
{
	return 0;
}

int CVisionComm_TestHand_2::OnReceiveS107F9(CPacket* pPacket)
{
...
// 		// SendMessage();
// 		g_pVisionTestHand_2->SetReceiveState(nHand, pRcvBody->stData.nStatus);
// 
// 		ST_PCD_BACKLASH_RCV_DATA vData;
// 		vData.nHand = nHand;
// 		vData.nState = pRcvBody->stData.nStatus;
// 		g_pVisionTestHand_2->SetReceiveData(vData);
// 
// 		return 0;
// 	}
// 
...
// 	strLog += strData;
// 	g_pVisionTestHand_2->SendLogMessage(strLog.GetString());
// 
// 	g_pVisionTestHand_2->SetReceiveData(vData);
// 
// 	g_pVisionTestHand_2->SetReceiveState(nHand, CAM_STATUS_SUCCESS);
// 
// 	g_pVisionTestHand_2->SendLogMessage("[Rcv] S107F9. MSGID : %d", pPacket->pHead->stData.nMsgId);
// 	return 0;
}

void CVisionComm_TestHand_2::SetReceiveState(int nHand, int nState)
{
	m_nIsReceived[nHand] = nState;
}

int CVisionComm_TestHand_2::GetReceiveState(int nHand)
{
...
void CVisionComm_TestHand_2::SetVisionComm(BOOL bState)
{
	m_bVisionComm = bState;
}

BOOL CVisionComm_TestHand_2::GetVisionComm()
{
	return m_bVisionComm;
}


int CVisionComm_TestHand_2::SendMeasure_PcdBacklash(int nMeasureType/*=enum eInspVAT_MeasureType*/, int nWorkPos/*=enum _eVAT_AREA*/)
{
...
void CVisionComm_TestHand_2::OnReceive_PcdBacklash(CPacket* pPacket)
{
	CPacketBody_S107F9* pRcvBody = (CPacketBody_S107F9*)pPacket->pBody;
	int nBodySize = pRcvBody->GetSize();
...
void CVisionComm_TestHand_2::SetReceiveData(ST_PCD_BACKLASH_RCV_DATA vRcvData)
{
	m_vRcvData[vRcvData.nHand] = vRcvData;
}

void CVisionComm_TestHand_2::GetReceiveData(int nAreaVAT, ST_PCD_BACKLASH_RCV_DATA &vRcvData)
{
	vRcvData = m_vRcvData[nAreaVAT];
}

int CVisionComm_TestHand_2::GetHandTypeIdx(_eTestPPIdx eTestPPIdx, eTestPPType eHandType)
{
	int nHandTypeIdx = -1;
...
int CVisionComm_TestHand_2::SendMeasure_VAT(CPacket* pPacket)
{
...
void CVisionComm_TestHand_2::OnReceive_VAT(CPacket* pPacket)
{
	//Data Receive 임계 영역 설정.
	g_pVisionTestHand_2->m_csRcv.Lock();
	...
	switch (pRcvBody->stData.nDataID)
	{
		case VISION_RCV_DATAID_VAT_TEST_2_LD_UPPER:		nCam = VAT_CAM_LD_TEST_PP_2_UPPER; break;
		case VISION_RCV_DATAID_VAT_TEST_2_ULD_UPPER:	nCam = VAT_CAM_UD_TEST_PP_2_UPPER; break;
		case VISION_RCV_DATAID_VAT_TEST_2_LOWER: 		nCam = VAT_CAM_TEST_PP_2_LOWER; break;
		default:
			break;
	}
	...
	if (!(VAT_CAM_LD_TEST_PP_2_UPPER <= nCam && nCam <= VAT_CAM_TEST_PP_2_LOWER)) {
		g_pVisionTestHand_2->SendLogMessage("[Rcv ERROR] [Cam ID Error] [CAM : %d]", nCam);
		g_pVisionTestHand_2->m_csRcv.Unlock();
		return;
	}	
