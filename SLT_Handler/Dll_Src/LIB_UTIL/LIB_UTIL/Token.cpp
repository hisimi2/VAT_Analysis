...
int Tokenize(LPCTSTR pszSeparate, CString strBuf, TokenArray& arrOut, int nMaxTkn/*=10*/)
{
	arrOut.clear();
	CString tkn;
	...
	/*string tokenize*/
	int nNextToken = 0;
	for (int i = 0; i < nMaxTkn; i++) {
		tkn = strBuf.Tokenize(pszSeparate, nNextToken);
		tkn.TrimLeft();
		tkn.TrimRight();
		arrOut.push_back(tkn);
		/*finish check*/
		if (strBuf.GetLength() <= nNextToken || nNextToken == (-1))
			break;
	}
	return arrOut.size();
}
