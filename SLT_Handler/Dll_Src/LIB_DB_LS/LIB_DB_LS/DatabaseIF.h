...
class AFX_EXT_CLASS CDatabaseIF
{
public:
	CDatabaseIF(void);
	~CDatabaseIF(void);
	...
	// Driver 시작과 종료
	int		Create(const char* szDBFileName, int nFileNameLen, int nType);
	void	Destroy();
	...
	int		SELECT_ERROR_LOG_TBL_LEVEL    (CString strDate_S, CString strDate_E, int nLevel, CMyList& rLogList);
	int		SELECT_ERROR_LOG_TBL_LEVEL    (int nLevel, CMyList& rLogList);
	int	    SELECT_ERROR_LOG_TBL_LEVEL(CString strDate_S, CString strDate_E, int nLevel, std::vector<ST_ERROR_MESSAGE>& vLogVector, BOOL nDaliyReport = FALSE, BOOL bChk_OnLine_OffLine = FALSE);
	...
	int		UPDATE_ADD_COLUMN_TBL(CString strAddColumnName, CString strAddColumnType, CString strSelTable = "ALL");
	int     DELETE_LOG_TBL_ALL();

	// TBL_TEACH_LOG
	int		INSERT_TEACH_LOG_TBL(_tTEACH_LOG& tTeachLog);
	int     SELECT_TEACH_LOG_TBL(CString strDate_S, CString strDate_E, std::vector<ST_TEACH_LOG>& vLogVector);
	// TBL_MOTOR_TEACHING
	...
	int     SELECT_DEVICE_DATA_TBL(int nTableNum, std::deque<ST_AAB_CHECK_DATA> &qDvcData);
	...
	int UPDATE_CYLINDER_TIME_TBL(int nIdx, double dCylinderValue);
