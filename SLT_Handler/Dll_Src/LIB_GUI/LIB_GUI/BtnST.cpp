...
// Parameters :
//		[IN]	nX
//				View Icon Origin Position X
//		[IN]	nY
//				View Icon Origin Position Y
//
void CButtonST::SetImageOrg(int nX, int nY)
{
	m_ptImageOrg.x = nX;
	m_ptImageOrg.y = nY;
}
...
//2008.1.21	JK
	if(bIsPressed == TRUE && m_bClickReDraw == FALSE) {
		bIsPressed = 0;
	}
...
DWORD CButtonST::SetIcon(HICON hIconIn, HICON hIconOut)
{
	...
	if (hIconIn)
	{
		...
		if (hIconOut)
		{
			...
			// Get icon dimension
			::ZeroMemory(&iii, sizeof(ICONINFO));
			bRetValue = ::GetIconInfo(hIconOut, &iii);
			if (bRetValue == FALSE)
			{
				FreeResources();
				return BTNST_INVALIDRESOURCE;
			...
			m_csIcons[1].dwWidth	= (DWORD)(iii.xHotspot * 2);
			m_csIcons[1].dwHeight	= (DWORD)(iii.yHotspot * 2);
			::DeleteObject(iii.hbmMask);
			::DeleteObject(iii.hbmColor);
		} // if
	} // if
...
void CButtonST::SetClickReDraw(BOOL bFlag)
{
	m_bClickReDraw = bFlag;
}
