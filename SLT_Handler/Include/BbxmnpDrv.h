...
#include "nmiMNApi.h"
#include "nmiMNApiDefs.h"
#include "DefineBbxmnp.h"
#include "MotionManager.h"
#include "ThreadBase.h"

class CBbxmnpDrv : public CWnd, public CThreadBase
{
// Function
public:
	CBbxmnpDrv(void);
	...
	void  SetParentCallbackAlarmReport(FN_PARENT_CALLBACK cbFunc);
	BOOL CheckCommunicationAlarm( int *pBoardConnectSts = NULL );

	int AxGetHomeSensorStatus(int nCardNum, int nSlaveNum, int *nORG);
	// Variable
public:

	FN_PARENT_CALLBACK m_fnAlarm;
	CWnd*			   m_pOwnCWnd;
private:
	CMotionManager m_motionManager;
	int m_nArrChkErrComBoard[128];
};
